...
class STSPTWEnv:
    ...
    def step(self, selected, visit_mask_only =True, out_reward = False, generate_PI_mask=False, pip_step=1):
        ...
        # shape: (batch, pomo)
        # Add stochastic noise to distance
        ...
        next_arrival_time = torch.max(self.current_time[:, :, None] + travel_time / self.speed,
    ...
    def _get_travel_distance(self):
        ...
        # Add stochastic noise to segment distances
    ...
    def generate_dataset(self, num_samples, problem_size, path):
        ...
        print("Save STSPTW dataset to {}".format(path))
...
def generate_stsptw_data(size, graph_size, rnds=None, time_factor=100.0, loc_factor=100, tw_duration="5075"):
    """
    Copyright (c) 2020 Jonas K. Falkner
    Copy from https://github.com/jokofa/JAMPR/blob/master/data_generator.py
    """
    ...
    return STSPTW_SET(node_loc=nloc,
                     node_tw=tw,
                     durations=tw[..., 1] - tw[..., 0],
